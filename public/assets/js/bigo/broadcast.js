$(function () {
    ...
    $('#profile_submit_e').on('click', function (e) {
        var title = $('#broadcast_title').val(),
            type = $('#broadcast_type').val();
        var category = type == 'live' ? $("#broadcast_category_live").val() : $("#broadcast_category_game").val();
        if (title == '') {
            successAlert('The name is necessary');
        } else if (name.length > 16) {
            successAlert('The name is too long');
        } else {
            $.post($(".recharge_wrap").data('savebroadcasturl'), {
                title: title,
                type: type,
                category: category,
            }, function (data) {
