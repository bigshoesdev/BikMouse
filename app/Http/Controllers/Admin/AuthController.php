...
class AuthController extends JoshController
{
    ...
    public function postSignin(Request $request)
    {
        try {
            ...
            if ($user = Sentinel::authenticate($request->only(['loginid', 'password']), $request->get('remember-me', false))) {
...
//                    ->log('LoggedIn');
                //activity log ends
                return redirect("admin/users")->with('success', trans('auth/message.login.success'));
            }

            $this->messageBag->add('loginid', trans('auth/message.account_not_found'));

        } catch (NotActivatedException $e) {
            $this->messageBag->add('loginid', trans('auth/message.account_not_activated'));
        } catch (ThrottlingException $e) {
            $delay = $e->getDelay();
            $this->messageBag->add('loginid', trans('auth/message.account_suspended', compact('delay')));
        }
